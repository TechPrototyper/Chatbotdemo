...
3. **OpenAI Backend**: Nutzt die Assistant API von OpenAI, basierend auf dem GPT-4 Turbo Modell, um die Konversationen zu verarbeiten. Der Assistant ist mit System-Prompts "programmiert" darauf, den Anwender rund um das Thema Energie in Hamburg zu unterstützen. Er ist angehalten, den Gesprächsfokus auch immer wieder dorthin zu lenken, und bei Erfolglosigkeit das Gespräch ggf. auch zu beenden. Das (Azure-) OpenAI Assistant API ist sehr flexibel, von seinen zahlreichen Funktionen werden hier in der Demo nur wenige genutzt. Besonders interessant ist die Trennung von Konversation und Run im neuen API, die dazu führt, dass man einen einzelnen Gesprächsstrang grundsätzlich auch mit mehreren LLMs bzw. Assistants führen kann. Die API ermöglicht ohne weitere Tools die Gestaltung komplexer Interaktionen zwischen Anwender, verschiedenen Modellen und verschiedenen Systemen wie internen Datenbanken, Vektordatenbanken, externen APIs. Das aktuelle Backend läuft in OpenAI; zwar bestand zur Entwicklungszeit für den Entwickler auch Zugriff auf Azure OpenAI, und lt. Microsoft Dokumentation ist das hier verwendete moderne Assistant API in mehreren Locations online, darunter z.B. France Central, Sweden Central und East US. Indes sind in der Karwoche 2024 alle Versuche, mit den Ressourcen von Azure OpenAI einen Assistant zu generieren, einstweilen gescheitert. Die Deployments wurden zwar durchgeführt, aber die Assistants haben nicht geantwortet; sie scheiterten schon in der Azure OpenAI Studio Oberfläche im Assistant Playground, und funktionierten über API Aufrufe leider auch nicht. Hier lässt sich aber mit Microsoft evtl. ein Ticket öffnen, um das Backend kurzfristig von OpenAI auf Azure OpenAI umzustellen. Wie man im Code sehen kann, sind aber die Dateien o_openai.py (OpenAI Backend) und azure_openai.py (Azure OpenAI Backend) praktisch identisch, vom Client Objekt und der Initialisierung abgesehen.
